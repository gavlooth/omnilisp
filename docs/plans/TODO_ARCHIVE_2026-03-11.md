...
Goal: turn the remaining post-hardening performance work into small,
measurable slices with explicit owners and validation paths.

Source: 2026-03-10 codebase optimization audit (`eval_boundary_*`,
`eval_dispatch_types.c3`, `jit_jit_apply_multi_prims.c3`,
`scheduler_offload_worker.c3`, `deduce_relation_*`).
...
- [x] Remove per-call heap allocation for small `TypeId` argument vectors in `src/lisp/eval_dispatch_types.c3`.
- [x] Remove per-candidate heap allocation for temporary constraint-binding buffers in `method_constraints_satisfied(...)`.
- [x] Remove per-call heap allocation for typed-prefix argument checks in `src/lisp/jit_jit_apply_multi_prims.c3`.
...
## Test Separation for Non-Memory vs Memory Changes

Goal: stop mixing lifetime/boundary coverage with syntax, compiler, and benchmark ownership so local runs can target the changed surface without dragging memory-heavy lanes into unrelated work.

### A. Inventory and Classification

- [x] Documented slice-to-group ownership map in this file and `docs/areas/memory-runtime.md`.
- [x] Record `basic` as the non-memory safety net slice (syntax/runtime/regression only).
- [x] Split memory ownership into explicit lanes:
  - `memory-lifetime` (alias, smoke ownership owner),
  - `memory-lifetime-smoke` (owner: boundary correctness),
  - `memory-lifetime-policy` (owner: policy/parse contract),
  - `memory-lifetime-soak` (owner: boundary saturation probes),
  - `memory-lifetime-bench` (owner: boundary benchmark),
  - `memory-stress` (owner: global stress),
  - `allocator-validation` (owner: AST arena correctness),
  - `allocator-bench` (owner: AST throughput/benchmark).
- [x] Mark each lisp test family as a single-owner lane:
  - boundary correctness families in `tests_memory_lifetime_smoke_suite_groups.c3`,
  - boundary policy parser contracts in `tests_memory_lifetime_boundary_policy_tests.c3`,
  - allocator validation in `tests_memory_lifetime_ast_arena_validation_tests.c3`,
  - allocator benchmarks in `tests_memory_lifetime_ast_arena_benchmark.c3`.
- [x] Identify and resolve mixed-ownership leftovers: smoke no longer directly owns policy token parse or AST arena validation tests.

### B. Remove Mixed Ownership From `memory-lifetime-smoke`

- [x] Move boundary policy token parsing tests out of `memory-lifetime-smoke` if they are parser/config contract tests rather than lifetime ownership tests.
- [x] Move `AstArena` validation tests out of `memory-lifetime-smoke` into a dedicated allocator-validation owner.
- [x] Keep `memory-lifetime-smoke` limited to boundary finalize/commit/scope-restore, env-copy/coroutine/yield, lane-barrier/root-fallback/graph-audit, and boundary transaction/ownership regressions.
- [x] Re-check and confirm no syntax, compiler, or benchmark-specific suites remain in smoke ownership.

### C. Split Allocator Validation From Allocator Benchmark

- [x] Create a dedicated non-benchmark allocator validation lane for `AstArena` correctness.
- [x] Keep allocator validation runnable without benchmark env flags.
- [x] Move parser/compiler/macro throughput loops into allocator benchmark ownership only.
- [x] Keep allocator benchmark entrypoints env-gated, but do not let the benchmark owner double as the validation owner.
- [x] Make the benchmark lane name explicit enough that developers do not confuse it with required correctness coverage.

### D. Normalize Top-Level Lisp Slice Names

- [x] Add explicit ownership slices:
  - `memory-lifetime-policy`,
  - `allocator-validation`,
  - `allocator-bench`.
- [ ] Rename legacy names so they are visible and intentional (`memory-soak`/`syntax` aliases if still used).
- [x] Keep backward-compatible alias `memory-lifetime` -> `memory-lifetime-smoke`.
- [x] Add deterministic warnings/errors for deprecated slice aliases after grace window ends.

### E. Tighten Global Gate Composition

- [x] Update `scripts/run_global_gates.sh` so default local-safe gates do not pull memory-heavy or benchmark-adjacent slices for non-memory work.
- [x] Make allocator validation a separate explicit gate entry instead of piggybacking on `memory-lifetime-smoke`.
- [x] Keep soak and benchmark lanes opt-in in the global gate runner.
- [x] Ensure the lisp slice list documents why each slice exists and when it must run.

### F. Host vs Container Policy

- [x] Keep host default lisp validation on `basic` unless a non-memory safety slice is explicitly selected.
- [x] Run all memory ownership lanes only in container (`memory-lifetime*`, `allocator-*`, `memory-stress`).
- [x] Keep `memory-lifetime-soak`, `memory-stress`, and full benchmark lanes container-only in both direct and global-gate paths.
- [ ] Document the minimum required container path for boundary/lifetime changes versus allocator-benchmark work.
- [ ] Stop implying that memory ownership tests are required for syntax/compiler-only changes in contributor docs.

### G. Documentation and Workflow

- [x] Update `docs/PROJECT_TOOLING.md` with the new slice taxonomy and explicit container-only ownership-lane policy.
- [x] Update `docs/areas/memory-runtime.md` so memory ownership does not imply syntax/compiler/benchmark coverage ownership.
- [ ] Record the slice split and rationale in `memory/CHANGELOG.md` once the code move lands.
- [ ] Add a short contributor rule: test lanes must follow ownership, not convenience bundling.

## Syntax Surface Cleanup and Hard-Fail Migration

- [x] Hard-fail grouped Scheme-style `let` bindings in favor of canonical flat-pair `let`.
- [x] Hard-fail grouped named `let` bindings in favor of canonical flat-pair named `let`.
- [x] Hard-fail removed `letrec` syntax with migration guidance to `let ^rec`.
- [x] Require `match` clauses to be explicit `(pattern result)` pairs with deterministic diagnostics for missing-result and extra-form cases.
- [x] Restrict `with-continuation` to explicit use inside `handle` clauses.
- [x] Hard-fail legacy implicit continuation capture in nested handler clause syntax.
- [x] Normalize shipped examples toward canonical flat-pair `let` syntax in `examples/demo.omni` and `examples/scicomp_demo.omni`.
- [x] Normalize remaining live docs/examples that still show legacy constructor-match clause spelling.
- [x] Decide whether malformed macro-emitted `let` bindings in `vte_let(...)` should graduate from silent `nil` degradation to a hard diagnostic.

## Macro Surface Simplification

Goal: replace multi-clause macro definitions with one stable macro model that does not mimic function overloading and does not depend on punctuation-heavy quasiquote syntax.

### M1. Design Lock

- [x] Write the macro-surface decision note and lock these rules:
  - [x] macros have one definition form only
  - [x] macros are single syntax transformers, not overloaded callable sets
  - [x] multi-clause macro definition syntax is deprecated and must not expand further
  - [x] function overloading remains a function-only feature
- [x] Define the canonical replacement model:
  - [x] `syntax-match` handles syntax-shape branching inside macro bodies
  - [x] `template` handles emitted syntax construction
  - [x] choose one interpolation surface:
    - [x] explicit `(insert x)` / `(splice xs)`, or
    - [ ] lightweight placeholders such as `$x` / `..$xs`
  - [x] reject Clojure-style backtick/comma/comma-at as the primary public model
- [x] Record explicit non-goals:
  - [x] no new macro-only clause-definition DSL
  - [x] no attempt to make macros dispatch like runtime functions
  - [x] no reader-level punctuation sugar unless introduced later as optional sugar over the canonical word-based model

### M2. Syntax Object and Matching Semantics

- [x] Define the syntax input contract for macro transformers:
  - [x] what a macro receives for head, args, and full form
  - [x] how symbols, literals, lists, arrays, and dict syntax are represented
  - [x] whether macro input is raw parsed syntax or normalized syntax
- [x] Specify `syntax-match` semantics:
  - [x] wildcard pattern
  - [x] literal-symbol / keyword match
  - [x] list destructuring
  - [x] rest capture (`..`)
  - [x] nested pattern matching
  - [x] deterministic failure diagnostics for malformed patterns
- [x] Specify identifier/hygiene expectations:
  - [x] inserted captured syntax preserves source identity where needed
  - [x] template-introduced bindings follow one explicit hygiene rule
  - [x] errors explain whether a name came from user syntax or macro-introduced syntax

### M3. Parser and AST Work

- [x] Add parser support for the chosen single macro definition surface.
- [x] Add parser support for `syntax-match`.
- [x] Add parser support for `template`.
- [x] Add parser support for the chosen interpolation form (`insert`/`splice` or placeholder syntax).
- [x] Ensure malformed `syntax-match` and `template` forms fail with parser-visible diagnostics.
- [x] Decide whether existing macro clauses lower into the new representation temporarily or remain as a separate legacy parse path during migration.
- [x] Harden template-datum parsing so unsupported tokens never degrade to non-advancing `nil` placeholders:
  - [x] make unsupported `parse_template_datum()` tokens fail explicitly instead of returning silent `nil`
  - [x] guarantee parser progress or error on every template-datum branch
  - [x] add focused regression coverage for `{...}` and `[...]` inside macro templates
  - [x] ensure stdlib/bootstrap macro parse failures surface as deterministic parse errors rather than hanging `register_stdlib()` / `--test-suite lisp`

### M3a. Fix Current Template-Datum Hang

- [x] Reproduce the current hang with a minimal macro template containing `{...}` inside a list form.
- [x] Reproduce the same non-progress bug with `[...]` inside a macro template.
- [x] Change `parse_datum_impl(...)` so unsupported template tokens never return a non-advancing placeholder from inside `parse_datum_list(...)`.
- [x] Decide the immediate behavior for `T_LBRACE` / `T_LBRACKET` in template-datum parsing:
  - [x] either parse them correctly in template mode, or
  - [ ] reject them with a deterministic parse error until full support lands
- [x] Add an invariant test for parser progress: template-datum parsing must either consume input or set `has_error`.
- [x] Add a focused stdlib/bootstrap regression:
  - [x] a macro definition with `signal raise { ... }` must fail or succeed deterministically
  - [x] it must never hang the loader
- [x] Add a direct CLI/test-suite regression proving malformed macro templates fail fast instead of wedging `--test-suite lisp`.

### M4. Expander and Runtime Support

- [x] Refactor macro expansion so one macro definition maps to one transformer path.
- [x] Implement `syntax-match` evaluation in the macro expander.
- [x] Implement `template` emission in the macro expander.
- [x] Ensure emitted syntax preserves location/diagnostic context where current infrastructure allows.
- [x] Add explicit internal helpers for capture, insertion, and splicing instead of ad hoc clause-matching code.
- [x] Keep expansion failures deterministic and phase-correct:
  - [x] bad pattern
  - [x] bad splice target
  - [x] missing binding in template
  - [x] malformed emitted form
  - [x] no matching macro clause reports deterministic expansion error

### M5. Stdlib and Core Macro Migration

- [x] Inventory existing multi-clause macros in stdlib and runtime-facing Omni code.
- [x] Classify each macro:
  - [x] can become a normal function (none in current shipped macro set)
  - [x] should become a single-clause macro with `syntax-match` (`when`, `unless`, `cond`, `with-defaults`, `stream-yield`)
  - [x] should be removed entirely (none in current shipped macro set)
- [x] Convert thin runtime-dispatch wrappers to functions first (no thin dispatch-wrapper macros found in current shipped inventory).
- [x] Convert remaining true syntax macros to the single-transformer model one by one.
- [x] Prioritize migration of public-facing macros before niche/internal helpers.
- [x] Re-check `cond`, `with-defaults`, `when`, `unless`, and other shipped macros for readability under the new surface.

### M6. Compatibility and Deprecation Path

- [x] Decide whether legacy multi-clause macros get:
  - [x] immediate hard failure, or
  - [ ] a staged deprecation window with deterministic warnings
- [x] If a grace window exists, add one translation/compatibility layer only, with a paired removal task in this TODO. (N/A: no grace window selected)
- [x] Add one deprecation diagnostic per legacy macro surface that points to the canonical replacement shape.
- [x] Remove the legacy multi-clause path after stdlib/docs/examples are migrated.

### M7. Tests

- [x] Add focused parser tests for:
  - [x] single macro definition syntax
  - [x] `syntax-match` patterns
  - [x] `template` emission syntax
  - [x] interpolation / splice error cases
- [x] Add macro expansion tests for:
  - [x] empty case
  - [x] fixed-arity match
  - [x] rest/splice match
  - [x] nested syntax match
  - [x] malformed output rejection
- [x] Add hygiene/identifier tests for macro-introduced bindings and captured user identifiers.
- [x] Add regression tests proving migrated stdlib macros preserve current behavior.
- [x] Add one negative test per removed legacy multi-clause public form.

### M8. Docs and Rollout

- [x] Update `docs/LANGUAGE_SPEC.md` with the canonical macro model.
- [x] Update syntax/reference docs with `syntax-match` and `template` examples.
- [x] Add migration guidance showing:
  - [x] multi-clause macro before/after examples
  - [x] “use a function instead” examples for thin wrappers
  - [x] readability guidance for templates
- [x] Update contributor guidance to state:
  - [x] do not add new multi-clause macros
  - [x] prefer functions unless syntax control is required
  - [x] prefer explicit word-based macro notation over punctuation-heavy sugar
- [x] Remove legacy examples from stdlib/docs/tests once the deprecation window closes. (Remaining mentions are intentional: migration before/after note and negative regression tests.)
