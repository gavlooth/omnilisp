...
are checked Vulkan `scale-f32` plus binary `add-f32`, `sub-f32`, `mul-f32`,
`div-f32`, `min-f32`, and `max-f32` over dense row-major `Float32` tensors;
arbitrary backend source compilation and fusion remain fail-closed.
