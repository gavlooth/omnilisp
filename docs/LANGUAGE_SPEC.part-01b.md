...
and execution is only through explicit `kernel/run`. The first shipped runner is
checked Vulkan `scale-f32` over dense row-major `Float32` tensors; arbitrary
