...
| `tensor/capture` | 1 | Return a non-executing `tensor-graph` plan for supported all-Vulkan `Float32` Tensor source/map expression graphs, including node ids, source/map nodes, scalar operands, output id, shape, and invalidation metadata |
