...
`ml/relu` applies `max(input, 0)` to `Float64` or `Float32` Tensor inputs. `ml/sigmoid`, `ml/tanh`, and tanh-approximation `ml/gelu` are canonical `ml/*` activation surfaces for `Float32` Tensor inputs; Float64 inputs fail closed. `ml/sum`, `ml/mean`, population `ml/variance`, and `ml/max` operate on CPU and Vulkan `Float64`/`Float32` tensors; `ml/logsumexp` operates on CPU `Float64`/`Float32` and Vulkan `Float32`; `ml/softmax`, `ml/cross-entropy`, and `ml/mean-squared-error` remain CPU-only. Reductions drop reduced axes, softmax preserves shape, losses return scalar tensors, and unsupported CUDA/Vulkan ML paths fail closed without CPU fallback.
