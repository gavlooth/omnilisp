...
`ml/relu` applies `max(input, 0)` to `Float64` or `Float32` Tensor inputs. `ml/sigmoid`, `ml/tanh`, and tanh-approximation `ml/gelu` are canonical `ml/*` activation surfaces for `Float32` Tensor inputs; Float64 inputs fail closed. `ml/sum`, `ml/mean`, population `ml/variance`, `ml/max`, and `ml/mean-squared-error` operate on CPU and Vulkan `Float64`/`Float32` tensors; `ml/logsumexp`, `ml/softmax`, and `ml/cross-entropy` operate on CPU `Float64`/`Float32` and Vulkan `Float32`. `ml/conv1d` takes dense NCW input and OCK kernel tensors plus stride/padding/dilation/groups; CPU supports `Float64`/`Float32`, Vulkan supports direct dense `Float32`, and broad `ml-convolution` remains false. Reductions drop reduced axes, softmax preserves shape, losses return scalar tensors, and unsupported CUDA/Vulkan ML paths fail closed without CPU fallback.
