...
`ml-linear`, `ml-linear-direct-float64`, `ml-linear-direct-float32`, `ml-neural-relu-float64`, `ml-neural-relu-float32`, `ml-neural-sigmoid-float64`, `ml-neural-sigmoid-float32`, `ml-neural-tanh-float64`, `ml-neural-tanh-float32`, `ml-neural-gelu-float64`, `ml-neural-gelu-float32`, `ml-reduction-float64`, `ml-reduction-float32`, `ml-conv1d-direct-float64`, `ml-conv1d-direct-float32`, `ml-conv2d-direct-float64`, `ml-conv2d-direct-float32`, `ml-convolution`, `ml-neural-map`,
...
`ml/relu` applies `max(input, 0)` to `Float64` or `Float32` Tensor inputs. `ml/sigmoid`, `ml/tanh`, and tanh-approximation `ml/gelu` are canonical `ml/*` activation surfaces for `Float32` Tensor inputs; Float64 inputs fail closed. `ml/sum`, `ml/mean`, population `ml/variance`, `ml/max`, and `ml/mean-squared-error` operate on CPU and Vulkan `Float64`/`Float32` tensors; `ml/logsumexp`, `ml/softmax`, and `ml/cross-entropy` operate on CPU `Float64`/`Float32` and Vulkan `Float32`. `ml/conv1d` takes dense NCW input and OCK kernel tensors; `ml/conv2d` takes dense NCHW input and OIHW kernel tensors; both use explicit stride/padding/dilation/groups, support CPU `Float64`/`Float32`, support direct dense Vulkan `Float32`, and keep broad `ml-convolution` false. Reductions drop reduced axes, softmax preserves shape, losses return scalar tensors, and unsupported CUDA/Vulkan ML paths fail closed without CPU fallback.
