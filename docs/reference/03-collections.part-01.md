...
`ml/relu` applies `max(input, 0)` to `Float64` or `Float32` Tensor inputs. `ml/sigmoid`, `ml/tanh`, and tanh-approximation `ml/gelu` are canonical `ml/*` activation surfaces for `Float32` Tensor inputs; Float64 inputs fail closed. `ml/sum`, `ml/mean`, population `ml/variance`, `ml/max`, `ml/logsumexp`, and `ml/softmax` operate on CPU `Float64`/`Float32` tensors; reductions drop reduced axes while softmax preserves shape. These ML primitives preserve dtype and fail closed for CUDA/Vulkan until backend axis kernels land.
