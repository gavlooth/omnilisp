...
`ml-linear`, `ml-linear-direct-float64`, `ml-linear-direct-float32`, `ml-neural-relu-float64`, `ml-neural-relu-float32`, `ml-neural-sigmoid-float64`, `ml-neural-sigmoid-float32`, `ml-neural-tanh-float64`, `ml-neural-tanh-float32`, `ml-neural-gelu-float64`, `ml-neural-gelu-float32`, `ml-reduction-float64`, `ml-reduction-float32`, `ml-conv1d-direct-float64`, `ml-conv1d-direct-float32`, `ml-conv2d-direct-float64`, `ml-conv2d-direct-float32`, `ml-pool2d-direct-float64`, `ml-pool2d-direct-float32`, `ml-layer-normalization-float64`, `ml-layer-normalization-float32`, `ml-convolution`, `ml-neural-map`,
...
`ml/relu` applies `max(input, 0)` to `Float64` or `Float32` Tensor inputs. `ml/sigmoid`, `ml/tanh`, and tanh-approximation `ml/gelu` are canonical `ml/*` activation surfaces for `Float32` Tensor inputs; Float64 inputs fail closed. `ml/sum`, `ml/mean`, population `ml/variance`, `ml/max`, and `ml/mean-squared-error` operate on CPU and Vulkan `Float64`/`Float32` tensors; `ml/logsumexp`, `ml/softmax`, `ml/cross-entropy`, and `ml/layer-normalization` operate on CPU `Float64`/`Float32` and Vulkan `Float32`. `ml/layer-normalization(input axis [epsilon])` preserves input shape, normalizes each explicit axis slice, and requires positive finite epsilon. `ml/conv1d` takes dense NCW input and OCK kernels; `ml/conv2d` takes dense NCHW input and OIHW kernels; `ml/max-pool2d` and `ml/avg-pool2d` take dense NCHW input, exclude padding from average divisors, support CPU `Float64`/`Float32`, and support direct dense Vulkan `Float32`; broad `ml-convolution` is true only for that explicit dense family. Reductions drop reduced axes, softmax and layer normalization preserve shape, losses return scalar tensors, and unsupported CUDA/Vulkan ML paths fail closed without CPU fallback.
