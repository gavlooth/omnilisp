...
`ml/relu` applies `max(input, 0)` to `Float64` or `Float32` Tensor inputs. `ml/sigmoid`, `ml/tanh`, and tanh-approximation `ml/gelu` are canonical `ml/*` activation surfaces for `Float32` Tensor inputs; Float64 inputs fail closed. `ml/sum`, `ml/mean`, and population `ml/variance` reduce one or more axes on CPU `Float64`/`Float32` tensors, drop reduced axes from the result shape, preserve dtype, and fail closed for CUDA/Vulkan until backend axis kernels land. These ML primitives report narrow capability bits while broad `ml-neural-map` remains false.
