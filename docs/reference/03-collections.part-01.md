...
rank-2 `Float64`, `Float32`, `Complex128`, or `Complex64` Tensors and returns
a dictionary with fixed-width complex `values` shape `[n]` and aligned
fixed-width complex `vectors` shape `[n n]`, with vector columns aligned to the
sorted values. `Float64` and `Complex128` inputs return `Complex128` values and
vectors; `Float32` and `Complex64` inputs return `Complex64` values and
vectors. Empty square matrices return empty `[0]` values and `[0 0]` vectors.
Runtime `LAPACKE_dgeev` / `sgeev` may accelerate real inputs when available,
with pure fallback for real inputs; `Complex128` and `Complex64` general
eigenpairs require `zgeev` / `cgeev` on CPU until a native non-Hermitian GPU
solver ships.
...
Backend entries report granular scientific keys alongside legacy broad keys:
`math-elementary-float64`, `math-elementary-float32`,
`math-error-function-float64`, `math-error-function-float32`,
`stats-distribution-float64`, and `stats-distribution-float32`. The
error-function keys cover `math.erf` and `math.erfc`, not `math.lgamma`.
The legacy `stats-normal-float64` and `stats-normal-float32` keys remain
compatibility aliases for `stats.normal-cdf` and `stats.normal-quantile`
coverage and do not imply every scientific unary map operation exists for the
dtype.
Every backend entry also exposes backend-neutral ML suite capability keys:
`ml-linear`, `ml-linear-direct-float64`, `ml-linear-direct-float32`, `ml-neural-relu-float64`, `ml-neural-relu-float32`, `ml-neural-sigmoid-float64`, `ml-neural-sigmoid-float32`, `ml-neural-tanh-float64`, `ml-neural-tanh-float32`, `ml-neural-gelu-float64`, `ml-neural-gelu-float32`, `ml-reduction-float64`, `ml-reduction-float32`, `ml-conv1d-direct-float64`, `ml-conv1d-direct-float32`, `ml-conv2d-direct-float64`, `ml-conv2d-direct-float32`, `ml-pool2d-direct-float64`, `ml-pool2d-direct-float32`, `ml-layer-normalization-float64`, `ml-layer-normalization-float32`, `ml-batch-normalization-float64`, `ml-batch-normalization-float32`, `ml-scaled-dot-product-attention-float64`, `ml-scaled-dot-product-attention-float32`, `ml-convolution`, `ml-neural-map`,
...
Vulkan keeps the broad key false while the narrow `ml-linear-direct-float64` and `ml-linear-direct-float32`
...
`ml/relu` applies `max(input, 0)` to `Float64` or `Float32` Tensor inputs. `ml/sigmoid`, `ml/tanh`, and tanh-approximation `ml/gelu` are canonical `ml/*` activation surfaces for `Float32` Tensor inputs; Float64 inputs fail closed. `ml/sum`, `ml/mean`, population `ml/variance`, `ml/max`, and `ml/mean-squared-error` operate on CPU and Vulkan `Float64`/`Float32` tensors; `ml/logsumexp`, `ml/softmax`, `ml/cross-entropy`, `ml/batch-normalization`, and `ml/scaled-dot-product-attention` operate on CPU `Float64`/`Float32` and Vulkan `Float32`; `ml/layer-normalization` operates on CPU and Vulkan `Float64`/`Float32`. `ml/layer-normalization(input axis [epsilon])` preserves input shape and normalizes each explicit axis slice; `ml/batch-normalization(input scale bias mean variance channel-axis [epsilon])` applies explicit rank-1 affine/stat tensors along the channel axis; `ml/scaled-dot-product-attention(query key value [mask] [scale])` computes max-shifted attention with optional additive masks; normalization epsilons and attention scale must be positive finite. `ml/conv1d` takes dense NCW input and OCK kernels; `ml/conv2d` takes dense NCHW input and OIHW kernels; `ml/max-pool2d` and `ml/avg-pool2d` take dense NCHW input, exclude padding from average divisors, support CPU `Float64`/`Float32`, and support direct dense Vulkan `Float32`; broad `ml-convolution` is true only for that explicit dense family. Reductions drop reduced axes, softmax, normalization, and attention preserve their contracted output shapes, losses return scalar tensors, and unsupported CUDA/Vulkan ML paths fail closed without CPU fallback. `ml/grad` currently supports CPU linear MSE, linear-activation MSE, linear softmax cross-entropy, tensor-expression MSE, and tensor-expression softmax cross-entropy specs; tensor-expression specs reverse-accumulate through supported dense CPU `map` and `contract` nodes, require one unambiguous `wrt` leaf, reject unsupported map backward rules, and return a metadata-only scope-owned `gradient-tape` dictionary with ordinary `nodes` and no native handles or executor state; `nn/forward`, `nn/grad`, and `nn/train-step` wrap that shipped dense training surface for train-mode model data and return updated ordinary data instead of hidden mutable state; `nn/sgd`, `nn/adam`, `nn/adamw`, and `nn/rmsprop` construct validated ordinary optimizer spec dictionaries; `Kernel(spec)` constructs a typed, inspectable custom kernel dictionary, `kernel/capture(kernel inputs push)` validates checked Vulkan direct-helper Kernel families and returns a single-node `kernel-graph` plan without launching, and `kernel/run(kernel inputs push)` supports checked Vulkan `scale-f32`, registered-source `source-scale-f32` using `source 'ml-clip-scale-f32`, direct-SPIR-V word-array `source-scale-f32` for the checked `source-scale-f32-v1` ABI, registered or direct-SPIR-V word-array unary `Float32` kernels for the checked `source-unary-f32-v1` ABI using registered source name `map-unary-f32`, direct-SPIR-V word-array binary `Float32` kernels for the checked `source-binary-f32-v1` ABI, binary `add-f32`, `sub-f32`, `mul-f32`, `div-f32`, `min-f32`, `max-f32`, scalar `add-scalar-f32`, `sub-scalar-f32`, `mul-scalar-f32`, `div-scalar-f32`, `min-scalar-f32`, `max-scalar-f32`, `scalar-sub-f32`, `scalar-div-f32`, and unary `abs-f32`, `neg-f32`, `sqrt-f32`, `identity-f32`, `zero-f32`, `sin-f32`, `cos-f32`, `tan-f32`, `asin-f32`, `acos-f32`, `atan-f32`, `sinh-f32`, `cosh-f32`, `tanh-f32`, `exp-f32`, `log-f32`, `log10-f32`, `erf-f32`, `erfc-f32`, `normal-cdf-f32` `Float32` tensor kernels while arbitrary source languages, arbitrary direct-SPIR-V descriptor layouts, and unsupported compilation/fusion routes fail closed; direct scale SPIR-V sources may declare `abi 'source-scale-f32-v1`, omitted ABI means the same checked scale ABI; direct unary SPIR-V sources must declare `abi 'source-unary-f32-v1`; direct binary SPIR-V sources must declare `abi 'source-binary-f32-v1`; checked direct scale, unary, and binary SPIR-V sources may declare `metadata` as a `kernel-source-layout` dictionary with matching ABI, descriptor layout, dtype, input/output counts, and optional push layout, while the legacy binary `metadata 'storage2-output1-f32-v1` spelling remains accepted for the checked binary ABI only; other direct-SPIR-V ABIs fail closed; `ml/sgd-step(parameters gradients learning-rate)` returns immutable CPU/Vulkan `Float64`/`Float32` parameter-tree updates, auto-migrating CPU leaves when the corresponding tree touches Vulkan. `ml/clip-gradients(gradients max-norm)` supports CPU `Float64`/`Float32` and all-Vulkan dense row-major `Float32` gradient trees, auto-migrating CPU `Float32` leaves when another leaf is Vulkan-placed; unsupported dtype trees fail closed. `ml/optimizer-step(spec parameters gradients state)` currently supports CPU/Vulkan dense row-major `Float64`/`Float32` SGD/Adam/AdamW/RMSProp and CUDA dense row-major `Float32` map-backed SGD/Adam/AdamW/RMSProp, with explicit returned state and optional `clip-norm` (applied before each optimizer update; CUDA and Vulkan clipping require matching dense `Float32` gradient paths); `ml/save-optimizer(spec state [path])` and `ml/load-optimizer(source)` checkpoint explicit optimizer spec/state dictionaries through the same envelope family as NN checkpoints; fused CUDA optimizer kernels remain fail-closed; Vulkan clipping support is reported through `ml-clip-gradients-float32`.
`ml/plot(data [options])` and `ml/loss-curve(losses [options])` return ordinary FTXUI graph nodes (`kind 'graph`, `props.series`) for finite numeric Array/List data and concrete CPU `Float64`/`Float32` tensor series. `ml/plot` additionally accepts x/y pairs, rank-2 `[N 2]` tensors, and Array/List multi-series overlays, recording `props.x` or `props.series-list` as appropriate; graph options may include a non-interactive data-window zoom dictionary with `x-min`, `x-max`, `y-min`, and `y-max`. `ml/scatter(points [options])`, `ml/bar-chart(values [options])`, `ml/histogram(values [options])`, `ml/roc-curve(points [options])`, and `ml/pr-curve(points [options])` return graph dictionaries with typed `props.plot-kind` metadata and lower to native FTXUI canvas rendering for markers, bars, histogram bars, multi-series overlays, and curve data. `ml/heatmap(matrix [options])` returns ordinary finite matrix metadata/data and lowers to a colored FTXUI canvas heatmap without using image display. Non-FTXUI graph backends fail closed. `ml/tensor-summary(tensor [options])` returns ordinary tensor summary data for any Tensor, including dtype/device/payload/layout/shape fields and direct CPU `Float64`/`Float32` min/max/mean stats when they are available; unsupported stats surfaces are reported in `stats.status`/`stats.reason` without device-to-CPU fallback. `ml/confusion-matrix(predictions targets [options])` returns ordinary target-row/predicted-column count data for equal-length integer class-id label series; tensor inputs must be rank-1 concrete CPU `Float64`/`Float32` labels and unsupported labels fail closed. `ml/export-image(tensor path [options])` writes a dependency-free PPM image file for rank-2 grayscale or rank-3 HWC `Float64`/`Float32` CPU tensors with 1 or 3 channels, finite non-negative unit or byte-range values, and returns ordinary export metadata; PNG/JPEG and non-CPU image export fail closed.
...
`tanh`, `exp`, `log`, `log10`, `math.erf`, `math.erfc`,
`stats.normal-cdf`, and `stats.normal-quantile` through dedicated unary helpers. `stats.normal-cdf`
...
`fusion-plan` data, top-level selected-region planner data, and `fusion 'none`. Launchable graphs record
`command-batching 'metadata` with one serial `command-buffer-candidate` batch
descriptor; non-launching graphs record `command-batching 'none`. The nested
memory plan records external/transient byte totals, per-node allocation
policy, and metadata-only transient reuse groups without allocating, retaining
handles, or reusing runtime buffers. The
...
dispatch. The selected-region plan records metadata-only native lowering
candidates for scalar-map chains, tensor-map chains, view-consuming chains,
contract-consuming chains, and the concrete mixed view/dense-source region,
including the command-batch candidate they consume. Captured graphs are not executed by capture, and capture does not
allocate, record, submit, or execute Vulkan command buffers. `tensor/run(graph)`
executes captured all-Vulkan `Float32` source/map, contract, and direct
transpose-view nodes by replaying them through existing Vulkan helpers;
eligible source -> scalar-map* graphs with two or more scalar map nodes,
eligible dense tensor/tensor map -> scalar-map* regions, and eligible
contract -> scalar-map* regions use matching `selected-region-plan` candidates
and validated `command-buffer-candidate` metadata to lower to native Vulkan
command-buffer batch/reuse helpers with native scratch ownership; arbitrary
command-buffer execution beyond those selected regions, hidden CPU fallback, and
`(define [kernel] ...)` sugar remain fail closed. Arbitrary strided view
graphs, mixed placement, unsupported map callables, unsupported dtypes, fused
dispatch, and general runtime buffer reuse remain fail closed. Vulkan
...
Direct `matrix/eigenpairs` supports Vulkan-placed dense row-major square
`Complex128` and `Complex64` inputs through native serial shifted-QR shaders,
returning Vulkan-placed fixed-width complex `values` and `vectors` without
hidden CPU/LAPACK fallback. Shader non-convergence raises
`tensor/no-convergence`. Real-valued Vulkan `Float64`/`Float32`
`matrix/eigenpairs` remains fail-closed; CPU keeps the LAPACK/pure fallback
