...
`fusion-plan` data, and `fusion 'none`. Launchable graphs record
...
memory plan records external/transient byte totals and per-node allocation
policy without allocating, retaining handles, or reusing runtime buffers. The
