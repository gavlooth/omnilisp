...
int omni_tensor_backend_vulkan_contract_scalar_chain_f32(
    ...
    *out_device_ptr = NULL;
    ...
    uint32_t contract_groups = 0u;
    result = omni_tensor_vulkan_group_count_1d(out_element_count, OMNI_TENSOR_VULKAN_CONTRACT_LOCAL_SIZE, &contract_groups);
    if (result != OMNI_TENSOR_VULKAN_SUCCESS) goto cleanup;
    ...
    uint32_t map_groups = 0u;
    result = omni_tensor_vulkan_group_count_1d(out_element_count, OMNI_TENSOR_VULKAN_MAP_LOCAL_SIZE, &map_groups);
    if (result != OMNI_TENSOR_VULKAN_SUCCESS) goto cleanup;
