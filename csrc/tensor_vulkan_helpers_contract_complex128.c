...
int omni_tensor_backend_vulkan_contract_complex128(
    ...
    *out_device_ptr = NULL;
    ...
    size_t element_size = sizeof(double) * 2u;
    size_t left_byte_len = 0;
    size_t right_byte_len = 0;
    size_t out_byte_len = 0;
    int byte_status = omni_tensor_vulkan_checked_element_bytes(left_count, element_size, &left_byte_len);
    if (byte_status != OMNI_TENSOR_VULKAN_SUCCESS) return byte_status;
    byte_status = omni_tensor_vulkan_checked_element_bytes(right_count, element_size, &right_byte_len);
    if (byte_status != OMNI_TENSOR_VULKAN_SUCCESS) return byte_status;
    byte_status = omni_tensor_vulkan_checked_element_bytes(out_element_count, element_size, &out_byte_len);
    if (byte_status != OMNI_TENSOR_VULKAN_SUCCESS) return byte_status;
    ...
    result = omni_tensor_vulkan_group_count_1d(out_element_count, OMNI_TENSOR_VULKAN_CONTRACT_LOCAL_SIZE, &group_count);
