...
int omni_tensor_backend_vulkan_svd_validate_shape_complex(
    ...
    if (out_k == NULL || out_u_count == NULL || out_u_storage_count == NULL ||
        out_s_storage_count == NULL || out_v_count == NULL || scalar_size == 0u) {
        return OMNI_TENSOR_VULKAN_INVALID;
    }
    *out_k = 0u;
    *out_u_count = 0u;
    ...
    *out_k = k;
    ...
    if (k > (UINT32_MAX - 128u) / 64u) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    if (rows != 0 && k > SIZE_MAX / rows) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    size_t u_count = rows * k;
    if (cols != 0 && k > SIZE_MAX / cols) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    size_t v_count = cols * k;
    ...
    if (k != 0u && k > SIZE_MAX / k) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    size_t gram_count = k * k;
    if (k > (SIZE_MAX - gram_count) / 2u) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    size_t hidden_count = gram_count + k * 2u;
    if (u_count > SIZE_MAX - hidden_count) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    size_t u_storage_count = u_count + hidden_count;
    if (u_storage_count > UINT32_MAX) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    if (k > SIZE_MAX - 1u) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    size_t s_storage_count = k + 1u;
    size_t complex_scalar_size = scalar_size * 2u;
    if (scalar_size > SIZE_MAX / 2u ||
        u_storage_count > SIZE_MAX / complex_scalar_size ||
        s_storage_count > SIZE_MAX / scalar_size ||
        v_count > SIZE_MAX / complex_scalar_size) {
        return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    }
    *out_u_count = u_count;
    *out_u_storage_count = u_storage_count;
...
int omni_tensor_backend_vulkan_svd_complex128(
    const void* input_device_ptr,
    size_t byte_len,
    size_t rows,
    size_t cols,
    ...
    *out_v_device_ptr = NULL;
    ...
    int status = omni_tensor_backend_vulkan_dispatch_one_input_three_outputs(
        input_device_ptr,
        byte_len,
        u_storage_count * sizeof(double) * 2u,
        s_storage_count * sizeof(double),
        v_count * sizeof(double) * 2u,
        1u,
        omni_tensor_vulkan_svd_complex128_spv,
        omni_tensor_vulkan_svd_complex128_spv_size,
        &push,
        sizeof(push),
        OMNI_TENSOR_VULKAN_SVD_LOCAL_SIZE,
        &u_device,
