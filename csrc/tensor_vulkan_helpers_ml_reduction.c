...
static int omni_tensor_backend_vulkan_ml_reduction_typed(
    ...
    *out_device_ptr = NULL;
    ...
    if (op > 2u) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    ...
    uint32_t group_count = ((uint32_t)out_element_count + OMNI_TENSOR_VULKAN_ML_REDUCTION_LOCAL_SIZE - 1u) / OMNI_TENSOR_VULKAN_ML_REDUCTION_LOCAL_SIZE;
