...
long g_omni_tensor_vulkan_map_view_scalar_chain_dispatch_call_count = 0;

long omni_tensor_backend_vulkan_map_view_scalar_chain_dispatch_call_count(void) {
    return g_omni_tensor_vulkan_map_view_scalar_chain_dispatch_call_count;
}

static void omni_tensor_vulkan_view_cmd_barrier_one_buffer(
...
static int omni_tensor_vulkan_view_dense_strides(
    size_t rank,
    const size_t* shape,
    size_t* strides,
    size_t* out_count
) {
    if ((rank > 0 && (shape == NULL || strides == NULL)) || out_count == NULL) return OMNI_TENSOR_VULKAN_INVALID;
    size_t count = 1;
    for (size_t cursor = rank; cursor > 0; cursor--) {
        size_t axis = cursor - 1;
        strides[axis] = count;
...
static int omni_tensor_vulkan_map_view_write_metadata(
    ...
    size_t* out_metadata_byte_len
) {
    if (out_metadata_buffer == NULL || out_metadata_byte_len == NULL) return OMNI_TENSOR_VULKAN_INVALID;
    *out_metadata_buffer = NULL;
    *out_metadata_byte_len = 0;
    ...
    size_t metadata_words_actual = out_rank * 2u;
    if (metadata_words_actual > SIZE_MAX - left_rank * 2u) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    metadata_words_actual += left_rank * 2u;
    if (metadata_words_actual > SIZE_MAX - right_rank * 2u) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    metadata_words_actual += right_rank * 2u;
    ...
    size_t metadata_byte_len = metadata_words * sizeof(uint32_t);
    ...
    status = omni_tensor_backend_vulkan_create_buffer_on_context(context, metadata_byte_len, &metadata_buffer);
    if (status != OMNI_TENSOR_VULKAN_SUCCESS) {
        free(metadata);
        return status;
    }
    void* mapped = NULL;
    if (omni_vulkan_map_memory(device, metadata_buffer->memory, 0, (OmniVulkanDeviceSize)metadata_byte_len, 0, &mapped) != OMNI_VULKAN_SUCCESS || mapped == NULL) {
        free(metadata);
        omni_tensor_backend_vulkan_destroy_buffer_handle(metadata_buffer);
        return OMNI_TENSOR_VULKAN_COPY_FAILED;
    }
    memcpy(mapped, metadata, metadata_byte_len);
    omni_vulkan_unmap_memory(device, metadata_buffer->memory);
    free(metadata);
    *out_metadata_buffer = metadata_buffer;
    *out_metadata_byte_len = metadata_byte_len;
    return OMNI_TENSOR_VULKAN_SUCCESS;
}

static void omni_tensor_vulkan_map_view_write_descriptor_set(
    OmniVulkanDevice device,
    OmniVulkanDescriptorSet set,
    OmniTensorVulkanBuffer* source,
    ...
    OmniVulkanDescriptorBufferInfo infos[4] = {
        { source->buffer, 0, (OmniVulkanDeviceSize)source->byte_len },
        { source->buffer, 0, (OmniVulkanDeviceSize)source->byte_len },
...
int omni_tensor_backend_vulkan_map_view_scalar_chain_f32(
    ...
    *out_device_ptr = NULL;
    ...
    if (element_count == 0 || input_byte_len == 0) return OMNI_TENSOR_VULKAN_SUCCESS;
    if (input_device_ptr == NULL || element_count > UINT32_MAX || element_count > SIZE_MAX / sizeof(float)) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    size_t output_byte_len = element_count * sizeof(float);
    if (input_byte_len < output_byte_len) return OMNI_TENSOR_VULKAN_INVALID;
    ...
    if (input_required > SIZE_MAX / sizeof(float) || input_required * sizeof(float) > input_byte_len) return OMNI_TENSOR_VULKAN_INVALID;
    ...
    status = omni_tensor_vulkan_view_dense_strides(rank, shape, dense_strides, &dense_count);
    if (status != OMNI_TENSOR_VULKAN_SUCCESS) {
        free(dense_strides);
        return status;
    }
    ...
    OmniTensorVulkanBuffer** metadata = (OmniTensorVulkanBuffer**)calloc(op_count, sizeof(OmniTensorVulkanBuffer*));
    size_t* metadata_byte_lens = (size_t*)calloc(op_count, sizeof(size_t));
    if (outputs == NULL || metadata == NULL || metadata_byte_lens == NULL) {
        free(dense_strides);
        free(outputs);
        free(metadata);
        free(metadata_byte_lens);
    ...
    for (size_t i = 0; i < op_count; i++) {
        ...
        const size_t* tensor_strides = i == 0 ? view_strides : dense_strides;
        size_t left_rank = modes[i] == 1u ? 0 : rank;
        size_t right_rank = modes[i] == 0u ? 0 : rank;
        status = omni_tensor_vulkan_map_view_write_metadata(
            device,
            context,
            rank,
            shape,
            dense_strides,
            left_rank,
            left_rank == 0 ? NULL : shape,
            left_rank == 0 ? NULL : tensor_strides,
            right_rank,
            right_rank == 0 ? NULL : shape,
            right_rank == 0 ? NULL : tensor_strides,
            &metadata[i],
            &metadata_byte_lens[i]
        );
        if (status != OMNI_TENSOR_VULKAN_SUCCESS) goto early_cleanup;
    }
    ...
    for (size_t i = 0; i < op_count; i++) {
        ...
        omni_tensor_vulkan_map_view_write_descriptor_set(device, descriptor_sets[i], source, outputs[i], metadata[i], output_byte_len, metadata_byte_lens[i]);
    }
    ...
    for (size_t i = 0; i < op_count; i++) {
        ...
        if (i + 1 < op_count) omni_tensor_vulkan_view_cmd_barrier_one_buffer(command_buffer, outputs[i], output_byte_len);
    }
    ...
    g_omni_tensor_vulkan_map_view_scalar_chain_dispatch_call_count++;
