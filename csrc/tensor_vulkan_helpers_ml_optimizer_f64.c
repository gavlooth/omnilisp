...
#define OMNI_TENSOR_VULKAN_ML_RMSPROP_LOCAL_SIZE 64u
typedef struct OmniTensorVulkanMlSgdPushConstantsF64 {
    uint32_t count;
    double learning_rate;
    double momentum;
    double weight_decay;
} OmniTensorVulkanMlSgdPushConstantsF64;

typedef struct OmniTensorVulkanMlAdamPushConstantsF64 {
    uint32_t count;
    double learning_rate;
    double beta1;
    double beta2;
...
} OmniTensorVulkanMlAdamPushConstantsF64;

typedef struct OmniTensorVulkanMlRmspropPushConstantsF64 {
    uint32_t count;
    double learning_rate;
    double alpha;
    double epsilon;
    ...
    double weight_decay;
    uint32_t has_square;
    uint32_t has_velocity;
} OmniTensorVulkanMlRmspropPushConstantsF64;

static int omni_tensor_vulkan_sgd_dispatch_two_outputs_f64(
    ...
    size_t shader_size,
    const OmniTensorVulkanMlSgdPushConstantsF64* push,
    ...
    *out_velocity_device_ptr = NULL;
    ...
    if (params_device_ptr == NULL || grads_device_ptr == NULL ||
        ...
        element_count > UINT32_MAX || element_count > SIZE_MAX / sizeof(double)) {
    ...
    if (result != OMNI_TENSOR_VULKAN_SUCCESS) goto cleanup_f64;
...
static int omni_tensor_vulkan_adam_dispatch_three_outputs_f64(
    const void* params_device_ptr,
    size_t params_byte_len,
    const void* grads_device_ptr,
    size_t grads_byte_len,
    ...
    const uint32_t* shader_words,
    size_t shader_size,
    const OmniTensorVulkanMlAdamPushConstantsF64* push,
    int has_moments,
    void** out_params_device_ptr,
    ...
    *out_second_device_ptr = NULL;
    ...
    if (queue == NULL) {
        result = OMNI_TENSOR_VULKAN_EXECUTION_FAILED;
        goto cleanup_adam_f64;
    }

    result = omni_tensor_vulkan_create_storage_descriptor_set_layout(device, descriptor_count, &descriptors);
    if (result != OMNI_TENSOR_VULKAN_SUCCESS) goto cleanup_adam_f64;
...
static int omni_tensor_vulkan_rmsprop_dispatch_f64(
    const void* params_device_ptr,
    size_t params_byte_len,
    const void* grads_device_ptr,
    size_t grads_byte_len,
    ...
    size_t velocity_byte_len,
    size_t element_count,
    const OmniTensorVulkanMlRmspropPushConstantsF64* push,
    void** out_params_device_ptr,
    void** out_square_device_ptr,
    void** out_velocity_device_ptr
) {
    if (out_params_device_ptr == NULL || out_square_device_ptr == NULL || out_velocity_device_ptr == NULL) return OMNI_TENSOR_VULKAN_INVALID;
    *out_params_device_ptr = NULL;
    *out_square_device_ptr = NULL;
    *out_velocity_device_ptr = NULL;
    if (!omni_tensor_backend_vulkan_available()) return OMNI_TENSOR_VULKAN_UNAVAILABLE;
    if (!omni_tensor_backend_vulkan_float64_available()) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    if (element_count == 0) return OMNI_TENSOR_VULKAN_SUCCESS;
    if (params_device_ptr == NULL || grads_device_ptr == NULL ||
        params_byte_len == 0 || grads_byte_len == 0 || push == NULL ||
        element_count > UINT32_MAX || element_count > SIZE_MAX / sizeof(double)) {
        return OMNI_TENSOR_VULKAN_INVALID;
    }
    ...
    size_t byte_len = element_count * sizeof(double);
    if (params_byte_len < byte_len || grads_byte_len < byte_len) return OMNI_TENSOR_VULKAN_INVALID;
    if (push->has_square && (square_device_ptr == NULL || square_byte_len < byte_len)) return OMNI_TENSOR_VULKAN_INVALID;
    if (push->has_velocity && (velocity_device_ptr == NULL || velocity_byte_len < byte_len)) return OMNI_TENSOR_VULKAN_INVALID;

    OmniTensorVulkanBuffer* params = (OmniTensorVulkanBuffer*)params_device_ptr;
    OmniTensorVulkanBuffer* grads = (OmniTensorVulkanBuffer*)grads_device_ptr;
    OmniTensorVulkanBuffer* square = (OmniTensorVulkanBuffer*)square_device_ptr;
    OmniTensorVulkanBuffer* velocity = (OmniTensorVulkanBuffer*)velocity_device_ptr;
    OmniTensorVulkanContext* context = params->context;
    if (context == NULL || context->device == NULL ||
        grads->context != context ||
    ...
    if (push->has_square &&
        (square->context != context || square->buffer == NULL || square->byte_len < square_byte_len)) {
        return OMNI_TENSOR_VULKAN_INVALID;
    }
    if (push->has_velocity &&
        (velocity->context != context || velocity->buffer == NULL || velocity->byte_len < velocity_byte_len)) {
        return OMNI_TENSOR_VULKAN_INVALID;
    }
    ...
    int status = omni_tensor_backend_vulkan_create_buffer_on_context(context, byte_len, &output_params);
    if (status != OMNI_TENSOR_VULKAN_SUCCESS) return status;
    OmniTensorVulkanBuffer* output_square = NULL;
    status = omni_tensor_backend_vulkan_create_buffer_on_context(context, byte_len, &output_square);
    if (status != OMNI_TENSOR_VULKAN_SUCCESS) {
        omni_tensor_backend_vulkan_destroy_buffer_handle(output_params);
        return status;
    }
    ...
    status = omni_tensor_backend_vulkan_create_buffer_on_context(context, byte_len, &output_velocity);
    if (status != OMNI_TENSOR_VULKAN_SUCCESS) {
        omni_tensor_backend_vulkan_destroy_buffer_handle(output_square);
        omni_tensor_backend_vulkan_destroy_buffer_handle(output_params);
        return status;
    }
    ...
    OmniTensorVulkanStorageBufferDescriptor buffer_descriptors[7] = {
        { params->buffer, (OmniVulkanDeviceSize)params_byte_len },
        { grads->buffer, (OmniVulkanDeviceSize)grads_byte_len },
        { push->has_square ? square->buffer : params->buffer, (OmniVulkanDeviceSize)(push->has_square ? square_byte_len : params_byte_len) },
        { push->has_velocity ? velocity->buffer : params->buffer, (OmniVulkanDeviceSize)(push->has_velocity ? velocity_byte_len : params_byte_len) },
        { output_params->buffer, (OmniVulkanDeviceSize)byte_len },
        { output_square->buffer, (OmniVulkanDeviceSize)byte_len },
        { output_velocity->buffer, (OmniVulkanDeviceSize)byte_len }
    };
    ...
    if (queue == NULL) {
        result = OMNI_TENSOR_VULKAN_EXECUTION_FAILED;
        goto cleanup_rmsprop_f64;
    }

    result = omni_tensor_vulkan_create_storage_descriptor_set_layout(device, 7u, &descriptors);
    if (result != OMNI_TENSOR_VULKAN_SUCCESS) goto cleanup_rmsprop_f64;
    ...
    result = omni_tensor_vulkan_create_compute_pipeline_for_shader(device, pipeline_layout, omni_tensor_vulkan_ml_rmsprop_f64_spv, omni_tensor_vulkan_ml_rmsprop_f64_spv_size, &shader_module, &pipeline);
    if (result != OMNI_TENSOR_VULKAN_SUCCESS) goto cleanup_rmsprop_f64;
...
cleanup_rmsprop_f64:
    ...
    *out_params_device_ptr = output_params;
    *out_square_device_ptr = output_square;
    *out_velocity_device_ptr = output_velocity;
    return OMNI_TENSOR_VULKAN_SUCCESS;
}
...
int omni_tensor_backend_vulkan_ml_sgd_f64(
    ...
    *out_params_device_ptr = NULL;
    *out_velocity_device_ptr = NULL;
    if (!isfinite(learning_rate) || !isfinite(momentum) || !isfinite(weight_decay) || learning_rate < 0.0 || momentum < 0.0) {
        return OMNI_TENSOR_VULKAN_INVALID_ARGUMENT;
    }
    if (element_count > UINT32_MAX || element_count > SIZE_MAX / sizeof(double)) return OMNI_TENSOR_VULKAN_UNSUPPORTED;

    OmniTensorVulkanMlSgdPushConstantsF64 push = {
    ...
    if (momentum <= 0.0) {
        return omni_tensor_backend_vulkan_dispatch_three_buffer_f64(
            params_device_ptr,
            params_byte_len,
            grads_device_ptr,
            grads_byte_len,
            ...
            element_count,
            omni_tensor_vulkan_ml_sgd_f64_spv,
            omni_tensor_vulkan_ml_sgd_f64_spv_size,
            &push,
            sizeof(push),
    ...
    return omni_tensor_vulkan_sgd_dispatch_two_outputs_f64(
        params_device_ptr,
        params_byte_len,
        grads_device_ptr,
        grads_byte_len,
        ...
        element_count,
        has_velocity ? omni_tensor_vulkan_ml_sgd_momentum_f64_spv : omni_tensor_vulkan_ml_sgd_init_momentum_f64_spv,
        has_velocity ? omni_tensor_vulkan_ml_sgd_momentum_f64_spv_size : omni_tensor_vulkan_ml_sgd_init_momentum_f64_spv_size,
        &push,
        out_params_device_ptr,
...
int omni_tensor_backend_vulkan_ml_adam_f64(
    ...
    *out_second_device_ptr = NULL;
    if (!isfinite(learning_rate) || !isfinite(beta1) || !isfinite(beta2) || !isfinite(epsilon) ||
        ...
        learning_rate < 0.0 || beta1 < 0.0 || beta1 >= 1.0 || beta2 < 0.0 || beta2 >= 1.0 ||
        epsilon <= 0.0 || weight_decay < 0.0 ||
        first_correction < DBL_MIN || second_correction < DBL_MIN) {
        return OMNI_TENSOR_VULKAN_INVALID_ARGUMENT;
    }
    if (element_count > UINT32_MAX || element_count > SIZE_MAX / sizeof(double)) return OMNI_TENSOR_VULKAN_UNSUPPORTED;

    OmniTensorVulkanMlAdamPushConstantsF64 push = {
        (uint32_t)element_count,
        learning_rate,
        beta1,
        beta2,
    ...
    return omni_tensor_vulkan_adam_dispatch_three_outputs_f64(
        params_device_ptr,
        params_byte_len,
        grads_device_ptr,
        grads_byte_len,
        ...
        element_count,
        has_moments ? omni_tensor_vulkan_ml_adam_f64_spv : omni_tensor_vulkan_ml_adam_init_f64_spv,
        has_moments ? omni_tensor_vulkan_ml_adam_f64_spv_size : omni_tensor_vulkan_ml_adam_init_f64_spv_size,
        &push,
        has_moments,
        out_params_device_ptr,
...
int omni_tensor_backend_vulkan_ml_rmsprop_f64(
    ...
    *out_velocity_device_ptr = NULL;
    if (!isfinite(learning_rate) || !isfinite(alpha) || !isfinite(epsilon) || !isfinite(momentum) || !isfinite(weight_decay) ||
        learning_rate < 0.0 || alpha < 0.0 || alpha >= 1.0 || epsilon <= 0.0 || momentum < 0.0 || momentum >= 1.0 || weight_decay < 0.0) {
        return OMNI_TENSOR_VULKAN_INVALID_ARGUMENT;
    }
    if (element_count > UINT32_MAX || element_count > SIZE_MAX / sizeof(double)) return OMNI_TENSOR_VULKAN_UNSUPPORTED;

    OmniTensorVulkanMlRmspropPushConstantsF64 push = {
        (uint32_t)element_count,
        learning_rate,
        alpha,
        epsilon,
    ...
    return omni_tensor_vulkan_rmsprop_dispatch_f64(
        params_device_ptr,
        params_byte_len,
        grads_device_ptr,
        grads_byte_len,
        has_square ? square_device_ptr : NULL,
        has_square ? square_byte_len : 0,
        has_velocity ? velocity_device_ptr : NULL,
        has_velocity ? velocity_byte_len : 0,
        element_count,
        &push,
        out_params_device_ptr,
        out_square_device_ptr,
        out_velocity_device_ptr
    );
}
