...
int omni_tensor_backend_vulkan_contract_f64(
    ...
    *out_device_ptr = NULL;
    ...
    int byte_status = omni_tensor_vulkan_checked_element_bytes(left_count, sizeof(double), &left_byte_len);
    if (byte_status != OMNI_TENSOR_VULKAN_SUCCESS) return byte_status;
    byte_status = omni_tensor_vulkan_checked_element_bytes(right_count, sizeof(double), &right_byte_len);
    if (byte_status != OMNI_TENSOR_VULKAN_SUCCESS) return byte_status;
    byte_status = omni_tensor_vulkan_checked_element_bytes(out_element_count, sizeof(double), &out_byte_len);
    if (byte_status != OMNI_TENSOR_VULKAN_SUCCESS) return byte_status;
