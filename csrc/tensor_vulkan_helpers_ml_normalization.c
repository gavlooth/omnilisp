...
int omni_tensor_backend_vulkan_ml_layer_norm_f64(
    ...
    *out_device_ptr = NULL;
    ...
    if (input_rank == 0 || axis >= input_rank) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    if (input_shape == NULL || input_strides == NULL || out_strides == NULL) return OMNI_TENSOR_VULKAN_INVALID;
    if (!isfinite(epsilon) || epsilon <= 0.0) return OMNI_TENSOR_VULKAN_INVALID_ARGUMENT;
    if (input_rank > UINT32_MAX || input_element_count > UINT32_MAX) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    if (input_element_count > SIZE_MAX / sizeof(double)) return OMNI_TENSOR_VULKAN_UNSUPPORTED;

    size_t input_required = 0;
    int shape_status = omni_tensor_vulkan_dense_required_elements(input_rank, input_shape, input_strides, &input_required);
    if (shape_status != OMNI_TENSOR_VULKAN_SUCCESS) return shape_status;
    if (input_required > SIZE_MAX / sizeof(double) || input_required > input_element_count) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    if (input_byte_len < input_required * sizeof(double)) return OMNI_TENSOR_VULKAN_INVALID;
    if (input_shape[axis] == 0) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    if (input_shape[axis] > UINT32_MAX) return OMNI_TENSOR_VULKAN_UNSUPPORTED;
    ...
    size_t out_byte_len = input_element_count * sizeof(double);
    if (input_element_count == 0 || out_byte_len == 0) return OMNI_TENSOR_VULKAN_SUCCESS;
    if (input_device_ptr == NULL) return OMNI_TENSOR_VULKAN_INVALID;
    ...
    OmniTensorVulkanMlLayerNormPushConstantsF64 push = {
        (uint32_t)input_rank,
        (uint32_t)input_element_count,
        (uint32_t)axis,
        (uint32_t)input_shape[axis],
    ...
    status = omni_tensor_backend_vulkan_dispatch_three_buffer_f64(
        input_device_ptr,
        input_byte_len,
        metadata_buffer,
        metadata_words * sizeof(uint32_t),
        out_byte_len,
        input_element_count,
        omni_tensor_vulkan_ml_layer_norm_f64_spv,
        omni_tensor_vulkan_ml_layer_norm_f64_spv_size,
        &push,
        sizeof(push),
        OMNI_TENSOR_VULKAN_ML_REDUCTION_LOCAL_SIZE,
        out_device_ptr
...
static int omni_tensor_backend_vulkan_dispatch_batch_norm_f64(
    const void* input_device_ptr,
    size_t input_byte_len,
    const void* scale_device_ptr,
    size_t scale_byte_len,
    ...
    *out_device_ptr = NULL;
    ...
    result = omni_tensor_vulkan_create_compute_pipeline_for_shader(
        device,
        pipeline_layout,
        omni_tensor_vulkan_ml_batch_norm_f64_spv,
        omni_tensor_vulkan_ml_batch_norm_f64_spv_size,
        &shader_module,
        &pipeline
    );
    if (result != OMNI_TENSOR_VULKAN_SUCCESS) goto cleanup;
...
int omni_tensor_backend_vulkan_ml_batch_norm_f64(
    ...
    *out_device_ptr = NULL;
    ...
    size_t channel_dim = input_shape[channel_axis];
    size_t param_byte_len = channel_dim * sizeof(double);
    if (scale_byte_len < param_byte_len || bias_byte_len < param_byte_len ||
        mean_byte_len < param_byte_len || variance_byte_len < param_byte_len) {
        return OMNI_TENSOR_VULKAN_INVALID;
    }
    ...
    OmniTensorVulkanMlBatchNormPushConstantsF64 push = {
        (uint32_t)input_rank,
        (uint32_t)input_element_count,
        (uint32_t)channel_axis,
        (uint32_t)channel_dim,
    ...
    status = omni_tensor_backend_vulkan_dispatch_batch_norm_f64(
        input_device_ptr,
        input_byte_len,
        scale_device_ptr,
        scale_byte_len,
