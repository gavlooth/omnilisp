...
- Part 18: [docs/todo_parts/todo_part_18.md](docs/todo_parts/todo_part_18.md) (251 lines)
...
- The proof-driven memory-boundary optimizer roadmap in
  ...
  and closure residual classification. The measured follow-up queue is now open
  on `MEM-BOUNDARY-BIGINT-COPY-001`: counters-enabled
  `memory-lifetime-smoke` reports `BIG_INTEGER` as the remaining
  optimizer-addressable materialization bucket (`56/264` copied bytes). The
  closure bucket (`208/264`) is expected no-splice rollback coverage.
